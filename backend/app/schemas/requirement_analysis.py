...
class RequirementAnalysisResult(BaseModel):
    ...
    # This is not included in normal Pydantic serialization.
    _llm_metadata: dict = PrivateAttr(
    ...
    # Safe defaults:
    # If the LLM does not return these fields,
    ...
    def validate_users(
        ...
        for value in values:
            ...
            if not value:
                raise ValueError(
                    "User entries cannot be blank"
                )

            if len(value) > 200:
                raise ValueError(
                    "User entry is too long"
                )
